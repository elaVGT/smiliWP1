...
namespace ElaPositioningEngine.Controller
{
    public class iDataHandler
    {
        ...
        private readonly Dictionary<uint, ElaSmiliDataPacket> iPosDataList = new Dictionary<uint, ElaSmiliDataPacket>();
        /** internal collection of Vbat data*/
        private readonly Dictionary<uint, ElaSmiliDataPacket> iVBatDataList = new Dictionary<uint, ElaSmiliDataPacket>();
        ...
        public void update_iPosData(uint nodeID, ElaSmiliDataPacket resultDatapacket)
        {
            if (this.m_mutexIncoming.WaitOne())
            {
                if (iPosDataList.ContainsKey(nodeID))
                {
                    iPosDataList[nodeID] = resultDatapacket;
                }
                else
                {
                    iPosDataList.Add(nodeID, resultDatapacket);
                }
